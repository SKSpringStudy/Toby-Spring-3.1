...
이 경우 getConnection()을 추상 메서드로 변경하고, 사용자가 UserDao클래스를 상속하여 서브클래스에서 getConnection()을 구현하여 사용하게 하면 된다.
```java
...
하지만 이 방식도 문제점이 존재한다.
1. SimpleConnectionMaker의 이름이 문제다. <br/>만약 사용자가 만든 DB커넥션 제공클래스는 openConnection()이라는 메서드 이름을 사용한다면 UserDao 내에 있는 add(), get() 메서드의 커넥션을 가져오는 코드를 다 변경해야 한다.
2. DB커넥션을 제공하는 클래스가 어떤 것인지 UserDao가 구체적으로 알고 있어야 한다. <br/>UserDao에서 SimpleConnectionMaker클래스 타입의 인스턴스 변수를 정의해놓고 있으니 사용자가 다른 클래스를 구현하면 UserDao 자체를 수정해야 한다.

이러한 문제는 두 클래스가 너무 긴밀히 연결되어 있기 때문이다. <br/>
UserDao클래스가 DB커넥션을 가져오는 클래스에 대해 어떤 클래스가 쓰일지, 어떤 메서드를 가져오는지 이름까지 다 일일히 알고 있다. <br/>
즉, UserDao가 SimpleConnectionMaker클래스에 종속적이기 때문에 확장하기가 힘들어지는 것이다.
